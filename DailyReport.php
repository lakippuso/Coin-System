<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <link rel="stylesheet" href="DailyReport.css">
        <link rel="stylesheet" href="css/bootstrap.css" />
        <title>Daily Report</title>
        <script src="https://kit.fontawesome.com/b67f9dc31e.js" crossorigin="anonymous"></script>
    </head>

    <body>
        <div class="dashboard">
            <!--Sidebar-->
            <div class="sidebar">
                <div class="profile">
                    <center><img src="profile.png"></center>
                    <div class="name">John Leo Jovita</div>
                </div>
                <div class="side_profile">
                    <a href="#"><button>Profile</button></a>
                </div>
                <div class="side_dashboard">
                    <a href="Dashboard.html"><button>Dashboard</button></a>
                </div>
                <div class="dropdown">
                    <div class="side_report">
                        <button class="dropdown_report">Reports <i class="fas fa-caret-down"></i></button>
                    </div>
                    <div class="dropdown_choices">
                        <a href="DailyReport.html"><button>Daily Report</button></a>
                        <a href="History.html"><button>History</button></a>
                    </div>
                </div>
                <div class="side_logout">
                    <a href="Homepage.html"><button class="logout">Logout</button></a>
                </div>
            </div>

            <!--Heading-->
            <div class="header">
                <div class="hd_dashboard">
                    <div class="page_name">Daily Report</div>
                </div>
            </div>            
        </div>

         <!--2nd layer-->
        <div class="flex-container">
            <div class="calendar">
                <input type="datetime-local" name="start_date"> 
            </div>
            <div class="calendar">
                <input type="datetime-local"  name="start_date" placeholder="Date End">
            </div>
            <div class="search_button">
                <button><i class="fas fa-search"></i></button>
            </div>
            <div class="generate_button">
                <input type="button" name="generate" value="Generate Report">
            </div>
        </div>

        <div class="table-responsive-md">
            <table class="table table-striped table-hover table-bordered">
                <thead>
                    <tr>
                        <th scope="col">Machine ID</th>
                        <th scope="col">Daily Income</th>
                        <th scope="col">Monthly Income</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                    <tr>
                        <th scope="col">Machine_001</th>
                        <th scope="col">90,000</th>
                        <th scope="col">90,000</th>
                    </tr>
                </tbody>
            </table>
        </div>
        
    <script src="dropdown.js"></script>
    <script src="js/bootstrap.js"></script>
    <script src="js/jquery.js"></script>
    </body>
</html>