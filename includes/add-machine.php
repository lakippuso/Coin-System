<?php
    ...
    if(!mysqli_stmt_prepare($stmt, $sql)){
        ...
        if($row = mysqli_fetch_assoc($result)){
            ...
            //INSERT NEW MACHINE
            $sql =  "INSERT INTO machine_info (machine_name, machine_type, username, income , date_created) VALUES (?, ?, ?, 0, ?)";
            $stmt = mysqli_stmt_init($con);
            if(!mysqli_stmt_prepare($stmt, $sql)){
                echo 'Update error Machine!';
            }
            else{
                mysqli_stmt_bind_param($stmt, "ssss", $machine_name, $machine_type, $username, $date);
                mysqli_stmt_execute($stmt);
